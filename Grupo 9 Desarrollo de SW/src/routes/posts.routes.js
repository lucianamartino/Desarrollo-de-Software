...
router.get('/create', async (req, res) => {
    ...
    if(req.session.loggedin) {
        res.render('posts/createPost', { oficios, oficioSeleccionado, usuarioIdLog: login ? req.session.usuarioId : 'Debe iniciar sesión' }); 
    } else {
        res.redirect('/')
    }
});
...
router.get('/f/:nombreOficio', async (req, res) => {
    ...
    res.render('posts/filtrarPosts', { posts, oficios, nombreOficio, oficioSeleccionado, usuarioIdLog: login ? req.session.usuarioId : 'Debe iniciar sesión' }); // Renderizar la vista con los posts filtrados
})
...
router.get('/:id', async (req, res) => {
    ...
    res.render('posts/postDetalle', { post, oficios, oficioSeleccionado, usuarioIdLog: login ? req.session.usuarioId : 'Debe iniciar sesión' }); 
});
